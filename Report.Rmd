...
TODO: include note how mean daily temp was calculated.
...
From the graph above, we can see that the average temperature increases as the semester went on. This makes sense as the semester began in the winter and progressed into spring. Interestingly, we can see two extremely cold days on February 3rd (21 degrees) and February 23 (20 degrees) and one abnormally warm day on March 3 (56.5 degrees).
...
Here, we can see that the number of daily COVID cases dropped exponentially when comparing the beginning and end of the semester. This could be explained by increasing vaccination rates, people being more cautious, or 
people not caring enough to get tested. Regardless of the reason, we can clearly see a downwards trend in COVID cases.
...
The graph above shows a linear trend of decreasing positive test rates as the semester progresses. 

Thus, these two graphs support the hypothesis that COVID rates decreased over time. 
...
From the plots above, we can see that Zoom attendance varies wildly between different courses with many peaks and troughs. However, the one exception is CS 3200 where we can see that the number of Zoom attendees generally decreases as the semester went on. Otherwise, the other courses displayed a general trend of a relatively constant number of Zoom people throughout the semester. One common attribute shared among all the graphs was that the number of attendance dropped near March 1st. 
...
data$date_2 = as.Date(data$date_2, format = "%m/%d/%Y")
ggplot(data, aes(x=date_2)) +
geom_line(aes(y=mean_daily_atd_rate)) + labs(title = "Mean Daily Attendance Rate Over Time", subtitle = "Jan 18, 2022 to Mar 24, 2022", y = "Attendance Count", x = "Date")

ggplot(data, aes(x=date_2)) +
geom_line(aes(y=mean_daily_temp)) + labs(title = "Mean Daily Temperature Rate Over Time", subtitle = "Jan 18, 2022 to Mar 24, 2022", y = "Temperature", x = "Date")

ggplot(data, aes(x=date_2)) +
geom_line(aes(y=daily_confirmed_case_count)) + labs(title = "UofU Daily Confirmed Case Count Rate Over Time", subtitle = "Jan 18, 2022 to Mar 24, 2022", y = "Temperature", x = "Date")

ggplot(data, aes(x=date_2)) +
geom_line(aes(y=confirmed_case_count_7d_avg)) + labs(title = "UofU Confirmed Case Count 7 Day Average Over Time", subtitle = "Jan 18, 2022 to Mar 24, 2022", y = "Confirmed Case Count", x = "Date")

ggplot(data, aes(x=date_2)) +
geom_line(aes(y=positive_test_rate_7d_avg)) + labs(title = "UofU Positive Test Rate 7 Day Average Over Time", subtitle = "Jan 18, 2022 to Mar 24, 2022", y = "Positive Test Rate", x = "Date")
```
...
One big limitation of the study was the fact that the courses chosen to study were a convenience sample. These were the classes that we were in and thus were easily accessible to collect data from. The implications of this are that our sample probably does a poor job of representing the population of CS courses at the University of Utah. A better approach would have been to randomly choose classes from all the available CS hybrid courses offered at the University. Moreover, we could have increased our sample size to get data more representative of the population as a whole.
...
For CS 3200, the teacher actively 
